...
export const AltTextViewer: React.FC<AltTextViewerProps> = ({ altText, isLoading, error, hasImage }) => {
  ...
  if (isLoading) {
    ...
    content = <div className="text-center text-red-400 bg-red-900/50 p-4 rounded-lg">{error}</div>;
  } else if (altText) {
    content = (
      <div className="space-y-4">
        ...
        <div className="relative bg-slate-900/70 p-4 rounded-lg">
          ...
          <button 
            onClick={handleCopy}
            className="absolute top-3 right-3 p-2 rounded-md bg-slate-700 text-slate-400 hover:bg-slate-600 hover:text-slate-200 transition-colors"
  ...
  return (
    <div className="bg-slate-800/50 rounded-lg p-6 h-96 flex flex-col justify-center">
      {content}
    </div>
  );
};
